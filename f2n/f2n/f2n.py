...
class f2nimage:
        def __init__(self, numpyarray=None,
                ...
                # For now, no PIL image. Some manipulations are to be done before.
                self.pilimage = None

                # Will be set to true if choosen in makepilimage. Changes default colours.
                self.negative = False
...
#               return pythoncopy.deepcopy(self)        
        ...
        def __str__(self):
                ...
                if self.xa != 0 or self.ya != 0 or self.xb != self.origwidth or self.yb != self.origheight:
                        return_string.append('\nRegion : [{0}:{1}, {2}:{3}]'.format(self.xa, self.xb, self.ya, self.yb))
                
                return_string.extend([
                        '\nPixel type : {0}'.format(str(self.numpyarray.dtype.name)),
                        '\nCutoffs : z1 = {0}, z2={1}'.format(self.z1,self.z2)
                ])
                
                if self.pilimage != None:
                        return_string.extend([
                                '\nPIL image mode : {0}'.format(str(self.pilimage.mode)),
                                '\nPIL image shape : ({0},  {1})'.format(self.pilimage.size[0], self.pilimage.size[1])
                        ])
        ...
        def setzscale(self, z1='auto', z2='auto', nsig=3, samplesizelimit = 10000, border=300, satlevel=65000):
                ...
                if self.numpyarray.shape[0] > 3 * border and self.numpyarray.shape[1] > 3 * border:
                        if border > 0:
                                if self.verbose :
                                        print('For the stats I will leave a border {0} pixels'.format(border))
                        ...
                        calcarray = self.numpyarray.copy()
                        if self.verbose:
                                print('Image is too small for a border of {0}'.format(border))
        ...
        def makepilimage(self, scale = 'log', negative = False):
                ...
                if scale == 'log' or scale == 'lin':
                        ...
                        if self.verbose:
                                print('PIL range : [{0}, {1}]'.format(np.min(bwarray), np.max(bwarray)))
                        ...
                        if self.verbose:
                                        print('PIL image made with scale : {0}'.format(scale))
                        return 0
                ...
                if scale == 'clog' or scale == 'lin': # Rainbow !
                        ...
                        if self.verbose:
                                        print('PIL image made with scale : %s'.format(scale))
                        return 0
...
#               
        ...
        def tonet(self, outfile):
                ...
                self.checkforpilimage()
                if self.verbose :
                        print('Writing image to {0}...\n{1} x {2} pixels, mode {3}'.format(outfile, self.pilimage.size[0], self.pilimage.size[1], self.pilimage.mode))
...
def fromfits(infile, hdu = 0, verbose = True):
        ...
        if verbose :
                print('Input shape : ({0}, {1})'.format(pixelarrayshape[0], pixelarrayshape[1]))
...
def compose(f2nimages, outfile):
        ...
        for i, line in enumerate(f2nimages):
                for j, img in enumerate(line):
                        if img.verbose:
                                print('Checking line {0}, image {1} (verbose)...'.format(i+1, j+1))
        ...
        for i, line in enumerate(f2nimages):
                ...
                if len(set(heights)) != 1 :
                        print('Heights of the images in line {0} :'.format(i + 1))
                        print(heights)
                        raise RuntimeError('Heights of the images in line {0} are not compatible.'.format(i + 1))
        
        totheight = np.sum(np.array([line[0].pilimage.size[1] for line in f2nimages]))
        # Ok, now it should be safe to go for the composition :
        if verbose:
                print('Composition size : {0} x {1}'.format(totwidth, totheight))
        ...
        if verbose:
                print('Writing compositions to {0}...\n{1} x {2} pixels, mode {3}'.format(outfile, compoimg.size[0], compoimg.size[1], compoimg.mode))
